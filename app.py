...
def copy_module():
    # המשתנים שנקלטים מהפורמט שעובד לך מצוין!
    ...
    # ניהול שלבי בקשת הנתונים (הנוסחה המדויקת שלך)
    ...
    try:
        ...
        # התאמת הנתיבים לפורמט הרשמי של ימות המשיח (עם ivr2:/)
        shluha_src = ext_src.strip('/')
        shluha_dst = ext_dst.strip('/')
        ...
        # 2. העלאת הקובץ למערכת היעד - שרשור כל הפרמטרים בתוך ה-URL כנדרש בפורום!
